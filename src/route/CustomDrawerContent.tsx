...
const CustomDrawerContent: React.SFC<CustomDrawerContentProps> = (props) => {
  ...
  const { loading, error, data, refetch } = useQuery(query, {
    variables: { tokenAccess: accessToken, userToken: useruid },
  });
    ...
    <>
      <View style={styles.profile}>
        ...
        <Text note style={[styles.userName, { marginTop: 5 }]}>
          {data.getAccess.client.razon_social}
        </Text>
        <Text
          numberOfLines={1}
          note
        ...
        >
          {data.getAccess.client.email}
        </Text>
      </View>
      <DrawerContentScrollView {...props}>
        {data.getAccess.access.map((route: any, index: number) => (
          <DrawerItem
            key={index}
